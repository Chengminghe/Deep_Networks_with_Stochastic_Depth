...
class StochasticNet(Model):
	def __init__(self,input_shape,num_class,layers, p_L = 0.5):
		...
		self.resblocks['res_2_1'] = TwoLayerStochastic((3,3),32,keep_prob=(1-(1+self.L/3)/self.L*(1-self.p_L)),down_sampling=True)
		self.resblocks['res_3_1'] = TwoLayerStochastic((3,3),64,keep_prob=(1-(1+2*self.L/3)/self.L*(1-self.p_L)),down_sampling=True)
		for i in range(self.L//3 - 1):
			self.resblocks['res_1_%d'%(i+2)] = TwoLayerStochastic((3,3),16,keep_prob=(1-(i+2)/self.L*(1-self.p_L)))
			self.resblocks['res_2_%d'%(i+2)] = TwoLayerStochastic((3,3),32,keep_prob=(1-(i+(2+self.L/3))/self.L*(1-self.p_L)))
			self.resblocks['res_3_%d'%(i+2)] = TwoLayerStochastic((3,3),64,keep_prob=(1-(i+(2+2*self.L/3))/self.L*(1-self.p_L)))
