...
class StochasticNet302(Model):
	def __init__(self,input_shape,num_class,p_L = 0.5):
		super(StochasticNet302,self).__init__()
		self.p_L = p_L
		self.L = 150
		...
		self.resblocks['res_2_1'] = TwoLayerStochastic((3,3),32,keep_prob=(1-51/self.L*(1-self.p_L)),down_sampling=True)
		self.resblocks['res_3_1'] = TwoLayerStochastic((3,3),64,keep_prob=(1-101/self.L*(1-self.p_L)),down_sampling=True)
		for i in range(49):
			self.resblocks['res_1_%d'%(i+2)] = TwoLayerStochastic((3,3),16,keep_prob=(1-(i+2)/self.L*(1-self.p_L)))
			self.resblocks['res_2_%d'%(i+2)] = TwoLayerStochastic((3,3),32,keep_prob=(1-(i+52)/self.L*(1-self.p_L)))
			self.resblocks['res_3_%d'%(i+2)] = TwoLayerStochastic((3,3),64,keep_prob=(1-(i+102)/self.L*(1-self.p_L)))
