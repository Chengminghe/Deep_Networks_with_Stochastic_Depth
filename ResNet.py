...
class LeNet(Model):
    ...
    def __init__(self, input_shape, output_size=24):
        '''
        ...
        self.conv_layer_1 = Conv2D(filters=48, kernel_size=(10, 10), strides=(4,4), activation='tanh', input_shape=input_shape, padding="same",use_bias=True)

        self.avgpool_layer_1 = AveragePooling2D(pool_size=(2, 2), strides=(2, 2), padding='valid')
        self.conv_layer_2 = Conv2D(filters=128, kernel_size=(5, 5), strides=(1,1), activation='tanh', padding="same",use_bias=True)
        self.avgpool_layer_2 = AveragePooling2D(pool_size=(2, 2), strides=(2, 2), padding='valid')
        self.conv_layer_3 = Conv2D(filters=192, kernel_size=(3, 3), strides=(1,1), activation='tanh',  padding="same",use_bias=True)

        self.avgpool_layer_3 = AveragePooling2D(pool_size=(2, 2), strides=(2, 2), padding='valid')

        
        self.flatten_layer = Flatten()

        self.fc_layer_1 = Dense(1280, activation='tanh')
        self.fc_layer_2 = Dense(24, activation='softmax')        
        #############################################################
        #                          END TODO                         #                                              
        #############################################################
